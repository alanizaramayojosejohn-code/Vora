-- =============================================================================
-- SaasGym · Step 2 — RLS por rol (admin vs caja)
-- =============================================================================
-- Hasta ahora las RLS de tablas tenant solo filtraban por business_id, sin
-- chequear rol. Eso permitía que un user `caja` con su access token llamara
-- PostgREST directamente y modificara productos, planes, clientes, etc.
--
-- Esta migración separa lectura de escritura:
--   · SELECT  → admin + caja del tenant (super_admin pasa siempre).
--   · WRITE   → solo admin del tenant (super_admin pasa siempre).
--
-- Excepción: clients permite INSERT a caja (registrar cliente nuevo al vender
-- una membresía). UPDATE/DELETE quedan solo para admin.
--
-- Excepción: sales, client_memberships, attendance quedan write-locked salvo
-- super_admin. Todo el flujo normal pasa por RPCs SECURITY DEFINER que bypass
-- RLS. Esto cierra el agujero de "caja borra una venta directo via REST".
-- =============================================================================

-- 1. Helper nuevo: is_admin_in_business -------------------------------------
-- Devuelve true si el caller es super_admin o admin del business indicado.
-- Reemplaza el OR inline en cada policy de write.
-- =============================================================================
...
-- 5. membership_plan_services (sin business_id propio; checa via plan) -------
...
-- 6. clients: SELECT + INSERT abiertos; UPDATE/DELETE solo admin -------------
...
-- 7. sales / client_memberships / attendance: SELECT abierto, write solo
--    super_admin (todo el flujo va por RPCs SECURITY DEFINER) ----------------
