-- =============================================================================
-- SaasGym · Step 1 — Aplicar migration 20260428000000_soft_delete_and_cancellation
-- =============================================================================
-- Pega y ejecuta este script en el SQL Editor del dashboard. Es idempotente:
--  · `add column if not exists` evita romper si ya corriste partes.
--  · `create or replace` para vistas y funciones.
--  · `create index if not exists` para los índices nuevos.
--
-- Si ya tienes datos productivos (FatumNeo POS, GymTest gym, Pedro Test, etc.)
-- nada se borra: solo agrega columnas nullables y reescribe vistas/RPCs.
-- =============================================================================
...
create index if not exists idx_products_active         on public.products(business_id) where deleted_at is null;
create index if not exists idx_sales_active            on public.sales(business_id, created_at) where cancelled_at is null;
create index if not exists idx_cm_active_not_cancelled on public.client_memberships(business_id, end_date) where cancelled_at is null;
