  ...
  template: `
    @if (pwa.canInstall()) {
      <div class="fixed bottom-0 left-0 right-0 z-50 p-3 sm:p-4 pwa-slide-up">
        <div
          class="max-w-sm mx-auto bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-2xl shadow-2xl overflow-hidden"
        >
          <div class="flex items-center gap-3 px-4 pt-4 pb-3">
            <img
              src="img/logo_dark.png"
              class="w-11 h-11 rounded-xl object-contain bg-zinc-100 dark:bg-zinc-800 p-1 shrink-0"
              alt="Vora"
            />
            <div class="min-w-0">
          ...
          @if (pwa.isIos()) {
            <div class="px-4 pb-4">
              <p class="text-xs text-zinc-600 dark:text-zinc-400 mb-3 leading-relaxed">
                Toca el botón
                <span
                  class="inline-flex items-center gap-1 font-medium text-zinc-800 dark:text-zinc-200"
                >
                ...
                <span class="font-medium text-zinc-800 dark:text-zinc-200"
                  >"Agregar a pantalla de inicio"</span
                >
              </p>
              <button
                (click)="pwa.dismiss()"
